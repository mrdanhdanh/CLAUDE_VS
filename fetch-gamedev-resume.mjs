...
async function fetchWithTimeout(url, ms=15000){
  ...
  try{
    const res = await fetch(url, { headers: { 'User-Agent': 'YUNIE-last30days/1.0', 'Accept': 'text/html' }, signal: ctrl.signal });
    return res;
  ...
  for(let attempt=0; attempt<=retries; attempt++){
    try{
      ...
      if(!articleBody){
        ...
        if(bodyMatch){
          try{ articleBody = JSON.parse(`"${bodyMatch[1]}"`); }catch{ articleBody = bodyMatch[1].replace(/\\n/g,'\n').replace(/\\"/g,'"'); }
        }
      }
      if(!articleBody){
      ...
      if(articleBody.includes('<') && articleBody.includes('>')) articleBody = stripHtml(articleBody);
...
async function main(){
  console.log(`📚 Resume fetching Gamedev — checking existing...`);
  let existing = { version:1, exportedAt: new Date().toISOString(), registry:{}, chunks:[] };
  if(fs.existsSync(EXPORT_PATH)){
    existing = JSON.parse(fs.readFileSync(EXPORT_PATH,'utf8'));
  ...
  const existingGamedev = Object.keys(registry).filter(k=>k.startsWith('hackernoon-gamedev-'));
  console.log(`Existing: ${Object.keys(registry).length} books, ${chunks.length} chunks — gamedev ${existingGamedev.length}`);
  // Find which indices are missing
  const existingSlugs = new Set(existingGamedev.map(id=> registry[id].slug));
  ...
  for(let i=0;i<URLS.length;i++){
    ...
    if(existingSlugs.has(slug)){
      console.log(`[${i+1}/90] SKIP already have slug ${slug}`);
      skipped++;
      continue;
    }
    console.log(`\n[${i+1}/90] ${slug}`);
    try{
      ...
      if(bookChunks.length===0){ failed++; continue; }
      registry[id] = {
        id, name: bookName, type: "hackernoon", enabled: true, read: false, progress: 0,
        chunks: bookChunks.length, size: data.fullText.length, addedAt: new Date().toISOString(), pages: 1,
        source: "HackerNoon", link: url, author: data.author, published: data.datePublished, tags: data.tags, image: data.image, desc: data.desc, slug
      };
      chunks.push(...bookChunks);
      totalChunks += bookChunks.length;
      success++;
      if((success)%10===0 || i===URLS.length-1){
  ...
  console.log(`\n✅ Done in ${elapsed}s: ${success} new, ${skipped} skipped, ${failed} failed, ${totalChunks} new chunks`);
  ...
  try{ JSON.parse(fs.readFileSync(EXPORT_PATH,'utf8')); console.log('✅ JSON valid'); }catch(e){ console.error('❌ JSON invalid', e.message); }
  // quick search test
  try{
