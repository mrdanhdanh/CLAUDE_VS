...
**Checklist mini (không có = chưa được code):** `Explore quick` → `Clarify 1 câu` → ghi `.agent/plans/<task>/prd.md|design.md|plan.md` (mini 5 dòng) + `manage_todo_list` → mới code. Xem `docs/harness-flow.md`.
...
- **Fix bug:** `Reproduce` có evidence → `Root Cause` file:line + 5 Whys → `Fix` tối thiểu → `Verify` lại reproduce + `get_errors` affected files. Không reproduce = không edit. Xem `docs/knowleged.md` + `systematic-debugging` skill.
