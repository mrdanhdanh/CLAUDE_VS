...
- **Guard:** `tests/e2e/guard-redteam.spec.ts` (G1 quarantine corpus + G2 compressHits provenance + **D1–D7 delegation attenuation** — mở HOLD 14/09: policy v5, 3 deny rules subagent ⊆ parent + engine `--parent`/`withinParent`)
...
- ❌ Paste KN mà không re-check ID ngay trước khi ghi — 3 phiên song song cùng nhận KN-061 (14/09); trước paste: chạy `findNextKnId` + grep ID vừa cấp; bên phát hiện sau đổi ID + update toàn bộ self-refs (routing đổi 061→062→063 — double-yield) (KN-063 + KN-056 class).
