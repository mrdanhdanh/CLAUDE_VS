...
- **Guard:** `tests/e2e/guard-redteam.spec.ts` (G1 quarantine corpus + G2 compressHits provenance + **D1–D7 delegation attenuation** — mở HOLD 14/09: policy v5, 3 deny rules subagent ⊆ parent + engine `--parent`/`withinParent`; **G3 visual-injection alt-text — pending, cần `verify` actor/human**)
- **Amend 2026-09-15 — modality-general (GỘP đề xuất KN-067, không tách mới):** Meta Repeat-After-Me 07/09/2026: visual injection ASR **>80%** trên GPT-5.5/Qwen3.6 "where adaptive textual prompt injection fails" (transfer 43–66%; demo ghi đè `TOOLS.md` → RCE); Muse xây classifier riêng cho "injection via images/media" + "files downloaded" (Meta 08/09/2026). Gap harness: `context.mjs:32` text-only. `evaluate` dup=42.5 → **GỘP** (KN-062). Luật modality đã áp `cua-safety` §1 + `agent-governance` §8. Bug: `.agent/bugs/2026-09-15-visual-prompt-injection-kenh-anh-cung-la-kenh-inje/` · Doc: `docs/meta-research-deep-dive.md` §2.2.
