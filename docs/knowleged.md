...
| KN-058 | 2026-09-13 | GitHub front page báo \"Unable to render rich display — Cannot read properties of undefined (reading 'render')\" cho block mermaid duy nhất trong README (cùng nội dung ở blob page + docs/harness-flow.md vẫn render) | Race phía GitHub viewscreen (mermaid 11.17.2 trong iframe): parent gửi `ready:ack` đáp lại status `ready` sau render, nhưng ở môi trường bị ảnh hưởng ack rơi vào document MỚI (iframe bị replace sau render) chưa có data → `t.render()` trên `undefined`; nội dung mermaid không liên quan — crash xảy ra trước parse | Loại bỏ đường lỗi: front page dùng SVG tĩnh `<picture>` light/dark render sẵn (htmlLabels:false — portable trong `<img>`), nguồn mermaid giữ ở docs/harness-flow.md; guard `readme-guard.spec.ts` cấm mermaid trong README | `ui` `render` `github` `mermaid` `race` `verify` |
...
- **Cách sửa:** Không thể sửa race phía GitHub → **loại bỏ đường lỗi**: README front page chuyển mermaid → **SVG tĩnh** (render bằng mermaid 11.17.2, `htmlLabels:false` để portable trong `<img>`, không external font) + `<picture>` theo `prefers-color-scheme` (bản dark có fill tối cho 2 node custom-style). Nguồn mermaid giữ nguyên ở `docs/harness-flow.md`.
- **Guard:** `tests/e2e/readme-guard.spec.ts` (3 test: README cấm ` ```mermaid ` · SVG light/dark tồn tại + được `<picture>` tham chiếu · mermaid source còn ở `docs/harness-flow.md`)
- **Cách phòng tránh:**
