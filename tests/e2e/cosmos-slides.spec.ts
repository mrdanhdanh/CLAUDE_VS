 ...
 * - Slide 15 "Tương lai": dòng "Đã ship & gỡ khỏi backlog" + 4 đề tài mới
 *   (đồng bộ index.html#future-title sau commit 125ffc5; 2026-09-12: QEC → Light Echo; Cosmic Web → Gravitational Lensing; Hawking → Escape Velocity; Escape Velocity shipped (--trend gate); CMB Anisotropy shipped (stats --heatmap) → còn 4)
...
test('slide 15 — "Đã ship" + 4 đề tài mới, hết Multiverse picker', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', (e) => errors.push(String(e)));
  ...
  await expect(slide).toContainText('4 đề tài mới');
  for (const topic of ['Gravitational Lensing', 'LIGO', 'Light Echo', 'Wormhole']) {
  ...
  await expect(slide.locator('.eta-chip')).toHaveCount(4);
