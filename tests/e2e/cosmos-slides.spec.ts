 ...
 * - Slide 15 "Tương lai": dòng "Đã ship & gỡ khỏi backlog" + 5 đề tài mới
 *   (đồng bộ index.html#future-title sau commit 125ffc5; 2026-09-12: QEC → Light Echo; Cosmic Web → Gravitational Lensing; Hawking → Escape Velocity; Escape Velocity shipped (--trend gate) → còn 5)
...
test('slide 15 — "Đã ship" + 5 đề tài mới, hết Multiverse picker', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', (e) => errors.push(String(e)));
  ...
  for (const topic of ['Gravitational Lensing', 'CMB', 'LIGO', 'Light Echo', 'Wormhole']) {
    await expect(slide).toContainText(topic);
  }
  // QEC + Cosmic Web + Hawking + Escape Velocity đã ship (2026-09-12) → nằm trong dòng "Đã ship", không trong danh sách chờ
  ...
  await expect(slide.locator('.eta-chip')).toHaveCount(5);
