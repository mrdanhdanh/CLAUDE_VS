...
---
<!-- generated by harness-manager export-claude — DO NOT EDIT, edit .github/prompts/fixbug.prompt.md instead -->
...
1. `Read docs/knowleged.md` — đọc toàn bộ, đặc biệt **Bảng tóm tắt** + **Anti-patterns** + **Checklist phòng tránh chung**.
...
- **Explore delegate (tiết kiệm):** Chỉ delegate `Explore` subagent (quick) khi cần hiểu codebase rộng / chưa rõ vị trí bug. **Không delegate nếu bug nằm trong phạm vi nhỏ và agent có thể xác định trực tiếp bằng `Grep`/`Read`.** Tránh tốn agent call cho bug nhỏ (typo, null check, API mapping).
- `Grep` tìm code liên quan bug.
- Ghi **Steps to Reproduce** (1-2-3), **Expected** vs **Actual**, **Evidence** (log, screenshot, test fail).
- Nếu không reproduce được → hỏi user thêm info (`AskUserQuestion` max 2 câu) — không đoán. Nếu vẫn FAIL → **Reproduce Gate: STOP / ask** — không đoán fix.
...
- Xác định **file:line** gây bug (dùng `Grep`, `Read` chunk lớn).
...
1. Tạo `TodoWrite` 3-5 todos (3-7 từ/todo), vd:
   - `Reproduce bug + ghi bug.md`
   - `Fix root cause tại file:line`
   - `Verify + regression test`
   - `Cập nhật knowleged.md KN-XXX`
2. Với mỗi todo: `in-progress` → `Read` → `Edit` / `Edit (nhiều chỗ, nhiều file)` → `IDE diagnostics` **affected files** → fix ngay → `completed` (chỉ 1 `in-progress` tại 1 thời điểm). **Không scan toàn project sau từng edit nhỏ** — chỉ check file vừa sửa. Full scope `IDE diagnostics` để ở Phase 4.
...
- Chỉ `kết thúc task` khi Verify PASS + Learn đã ghi.
...
- **Explore tiết kiệm:** Không delegate `Explore` nếu bug nhỏ và xác định được trực tiếp bằng `Grep`/`Read`.
