...
user-invocable: "true"
---
<!-- generated by harness-manager export-claude — DO NOT EDIT, edit .github/skills/systematic-debugging/SKILL.md instead -->
...
- Build failures (`dotnet build` MSB3027, `IDE diagnostics`), performance, integration
...
node .github/harness/scripts/auto-learn.mjs log --error "message" --file "path" --title "slug"  # khi IDE diagnostics fail
...
1. **Read Error Messages Carefully**
   ...
   - `IDE diagnostics` affected files → ghi lại message chính xác
...
3. **Verify Fix — verification-before-completion gate (BẮT BUỘC fresh evidence)**
   ...
   - Dùng `IDE diagnostics` full scope (Phase 4) — khác Phase 3 chỉ check affected files
...
5. **AAR-Style Fix Benchmark (nếu nhiều cách fix)**
   ...
   3. Benchmark mỗi cái: build + test + IDE diagnostics + edge cases
...
- Lệnh: `IDE diagnostics` (affected → full scope), `dotnet build/test --nologo`, `git diff/log`, `PowerShell` sync
