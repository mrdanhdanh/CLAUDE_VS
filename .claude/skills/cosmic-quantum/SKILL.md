...
| `TodoWrite` (1 in-progress) | **Nguyên lý loại trừ Pauli** | 2 todo không chiếm cùng trạng thái — vi phạm là decoherence |
...
Công thức Lab: bất định $\Delta x \cdot \Delta p \ge \hbar/2$ ↔ `IDE diagnostics` chính xác thì `build` mờ; entropy $S = k \log W$ ↔ debt tăng theo số trạng thái không đo.
