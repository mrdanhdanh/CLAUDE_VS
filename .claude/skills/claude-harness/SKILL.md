...
---
<!-- generated by harness-manager export-claude — DO NOT EDIT, edit .github/skills/claude-harness/SKILL.md instead -->
...
- Muốn quy trình chuẩn như Claude Code Extension nhưng chạy trong Claude Code
...
### Phase 2: CLARIFY
- Nếu ý tưởng mơ hồ → `AskUserQuestion` (tối đa 3 câu, có options)
...
### Phase 5: PLAN
- Tạo `.agent/plans/<task>/plan.md` + `TodoWrite` (3-7 từ/todo, 5-10 todos)
...
- `Read` chunk lớn → `Edit` / `Edit (nhiều chỗ, nhiều file)`
...
- `IDE diagnostics` + `PowerShell` (lint/build/test) — `dotnet test --nologo`, `dotnet build --nologo`
...
- Chỉ khi PASS + fresh evidence mới `kết thúc task`
...
- Không bỏ Polish
- `TodoWrite` bắt buộc cho task >2 bước
- `AskUserQuestion` khi mơ hồ
- Ghi nhớ pattern vào `CLAUDE.md`
